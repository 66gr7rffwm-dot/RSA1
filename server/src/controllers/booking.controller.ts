...
export const estimateBookingPrice = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const { tripId, pickupLatitude, pickupLongitude, dropoffLatitude, dropoffLongitude } = req.body;

    if (!tripId || typeof pickupLatitude !== 'number' || typeof pickupLongitude !== 'number' ||
        typeof dropoffLatitude !== 'number' || typeof dropoffLongitude !== 'number') {
      res.status(400).json({ success: false, error: 'Missing required fields' });
      return;
    }
    ...
    // Get trip details
    const tripResult = await pool.query(
      `SELECT t.*, v.fuel_type, v.seating_capacity
       FROM trips t
    ...
    const partialDistance = await navigationService.calculatePartialDistanceFactor(
      {
    ...
    const passengerCost = pricingEngine.calculatePassengerCostForBooking(
      trip.base_trip_cost,
      partialDistance.partialDistanceFactor
    );

    res.json({
      success: true,
      data: {
        estimatedCost: passengerCost,
        partialDistanceFactor: partialDistance.partialDistanceFactor,
        partialDistanceKm: partialDistance.partialDistanceKm,
        fullTripDistanceKm: trip.total_distance_km,
      },
    });
  } catch (error) {
    next(new AppError('Failed to estimate booking price', 500));
  }
};
